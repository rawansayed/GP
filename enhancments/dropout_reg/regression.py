...
def dropout_reg(epochNum):
    ...
    class MonitorCallback(tflearn.callbacks.Callback):
        ...
        def on_epoch_end(self, training_state):
            ...
            if ((max( self.maxtestSP_second)<test_SPEAR_Corrval)):
                model.save(f"{save_location}/regModel/thirdBestModel/ClassificationModel.tfl")
                self.maxtestSP_second.append( test_SPEAR_Corrval)

            # here the best model with the best spearman corr we found and 10% overfitting
            if ((max( self.maxtestSP_first)<test_SPEAR_Corrval) and((train_SPEAR_Corrval-test_SPEAR_Corrval)<0.1) ):
                model.save(f"{save_location}/regModel/BestModel/ClassificationModel.tfl")
                self.maxtestSP_first.append( test_SPEAR_Corrval)
            ...
            df.to_csv(f"{save_location}/statistical_measures/REGAccuracies.csv")
        ...
        def on_train_end(self,training_state):
            ...
            df.to_csv(f"{save_location}/statistical_measures/FinalREGAccuracies.csv")
