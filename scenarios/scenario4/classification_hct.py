...
def createROCCurve(model,X_test,Y_test,model_name):
    ...
    np.save(f"{save_location}/statistical_measures/hct/fpr_{model_name}.npy",fpr)
    np.save(f"{save_location}/statistical_measures/hct/tpr_{model_name}.npy",tpr)
    plt.plot(fpr, tpr, linestyle='--', color='blue')
    plt.title('ROC curve')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive rate')
    plt.legend(loc='best')
    plt.savefig(f"{save_location}/statistical_measures/hct/ROCfor{model_name}",dpi=300)
    plt.close() 


def scenario4_hct(epochNum):

    # --------------- loading Data --------------- #
    files = ['hek293t_doench.episgt','hl60_xu.episgt','hela_hart.episgt']
    ...
    dataArr_inputs_train_hct116=np.load(f"./training_data/inputs_hct116_hart_CLS.npy")
    dataArr_labels_train_hct116=np.load(f"./training_data/labels_hct116_hart_CLS.npy")
    ...
    class MonitorCallback(tflearn.callbacks.Callback):
        ...
        def on_epoch_end(self, training_state):
            ...
            if ((max( self.maxtestAUC_first)<test_auc_val)):
                model.save(f"{save_location}/clsModel/hct/BestModel/ClassificationModel.tfl")
                self.maxtestAUC_first.append( test_auc_val)
                createROCCurve(model,X_test,Y_test,"hctBestModel")
            ...
            df.to_csv(f"{save_location}/statistical_measures/hct/ClsAccuracies.csv")
        ...
        def on_train_end(self,training_state):
            ...
            df.to_csv(f"{save_location}/statistical_measures/hct/ClsAccuraciesFinal.csv")
    ...
    # save the last model
    model.save(f"{save_location}/clsModel/hct/finalModel/ClassificationModel.tfl")
