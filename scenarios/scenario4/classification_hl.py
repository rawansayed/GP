...
def createROCCurve(model,X_test,Y_test,model_name):
    ...
    np.save(f"{save_location}/statistical_measures/hl/fpr_{model_name}.npy",fpr)
    np.save(f"{save_location}/statistical_measures/hl/tpr_{model_name}.npy",tpr)
    plt.plot(fpr, tpr, linestyle='--', color='blue')
    plt.title('ROC curve')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive rate')
    plt.legend(loc='best')
    plt.savefig(f"{save_location}/statistical_measures/hl/ROCfor{model_name}",dpi=300)
    plt.close() 


def scenario4_hl(epochNum):

    # --------------- loading Data --------------- #
    files = ['hct116_hart.episgt','hela_hart.episgt','hek293t_doench.episgt']
    ...
    dataArr_inputs_train_hl60=np.load(f"./training_data/inputs_hl60_xu_CLS.npy")
    dataArr_labels_train_hl60=np.load(f"./training_data/labels_hl60_xu_CLS.npy")
    ...
    dataArr_inputs_test     = np.concatenate((dataArr_inputs_train_hl60,dataArr_inputs_test_hl60))
    dataArr_labels_test     = np.concatenate((dataArr_labels_train_hl60,dataArr_labels_test_hl60))
    ...
    class MonitorCallback(tflearn.callbacks.Callback):
        ...
        def on_epoch_end(self, training_state):
            ...
            if ((max( self.maxtestAUC_first)<test_auc_val)):
                model.save(f"{save_location}/clsModel/hl/BestModel/ClassificationModel.tfl")
                self.maxtestAUC_first.append( test_auc_val)
                createROCCurve(model,X_test,Y_test,"hlBestModel")
            ...
            df.to_csv(f"{save_location}/statistical_measures/hl/ClsAccuracies.csv")
        ...
        def on_train_end(self,training_state):
            ...
            df.to_csv(f"{save_location}/statistical_measures/hl/ClsAccuraciesFinal.csv")
    ...
    # save the last model
    model.save(f"{save_location}/clsModel/hl/finalModel/ClassificationModel.tfl")
