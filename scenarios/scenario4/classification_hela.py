...
def createROCCurve(model,X_test,Y_test,model_name):
    ...
    np.save(f"{save_location}/statistical_measures/hela/fpr_{model_name}.npy",fpr)
    np.save(f"{save_location}/statistical_measures/hela/tpr_{model_name}.npy",tpr)
    plt.plot(fpr, tpr, linestyle='--', color='blue')
    plt.title('ROC curve')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive rate')
    plt.legend(loc='best')
    plt.savefig(f"{save_location}/statistical_measures/hela/ROCfor{model_name}",dpi=300)
    plt.close() 


def scenario4_hela(epochNum):

    # --------------- loading Data --------------- #
    files = ['hct116_hart.episgt','hl60_xu.episgt','hek293t_doench.episgt']
    ...
    dataArr_inputs_train_hela=np.load(f"./training_data/inputs_hela_hart_CLS.npy")
    dataArr_labels_train_hela=np.load(f"./training_data/labels_hela_hart_CLS.npy")
    ...
    dataArr_inputs_test     = np.concatenate((dataArr_inputs_train_hela,dataArr_inputs_test_hela))
    dataArr_labels_test     = np.concatenate((dataArr_labels_train_hela,dataArr_labels_test_hela))
    ...
    class MonitorCallback(tflearn.callbacks.Callback):
        ...
        def on_epoch_end(self, training_state):
            ...
            if ((max( self.maxtestAUC_first)<test_auc_val)):
                model.save(f"{save_location}/clsModel/hela/BestModel/ClassificationModel.tfl")
                self.maxtestAUC_first.append( test_auc_val)
                createROCCurve(model,X_test,Y_test,"helaBestModel")
            ...
            df.to_csv(f"{save_location}/statistical_measures/hela/ClsAccuracies.csv")
        ...
        def on_train_end(self,training_state):
            ...
            df.to_csv(f"{save_location}/statistical_measures/hela/ClsAccuraciesFinal.csv")
    ...
    # save the last model
    model.save(f"{save_location}/clsModel/hela/finalModel/ClassificationModel.tfl")
